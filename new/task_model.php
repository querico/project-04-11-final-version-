...
/************toggle display 'create by table' by sort key word************/	
	...
	if ($sort == 'parent') { // if sort == "parent" than do the rest
		$queryParent = "SELECT DISTINCT parent_task_id FROM task WHERE parent_task_id <> 0 AND task.status <> 4 AND staff_id = $staff_id"; // stop repeate parent task id only select task that hase parent task and also 
			...
			foreach ($parentID as $element){
				...
				$querySelectParent = "SELECT task.task_id AS TaskID, staff.name AS CreateBy, 
						...
						$stmt = $this->mysqli->prepare($querySelectParent);					
						if($stmt) {
							if ($stmt->execute()){
									$result = $stmt->get_result();
